class argHandler(dict):
    ...
    def setDefaults(self):
        ...
        self.define(
            'load', '',
            'how to initialize the net? Either from .weights or a checkpoint, '
    ...
    def help(self):
        print('Example usage: flow --imgdir sample_img/ --model cfg/yolo.cfg '
        ...
        while i < len(args):
            ...
            if isinstance(self.get(argumentName), bool):
                if not (i + 1) >= len(args) and (
                    ...
                    print('ERROR - Expected boolean value (or no value) '
                          'following argument: ' + args[i])
                    print('Try running flow --help')
                    exit()
            ...
            elif args[i].startswith('--') and not (
                ...
                if isinstance(self[argumentName], float):
                    try:
                        ...
                        print('ERROR - Expected float for argument: ' +
                              args[i])
                        print('Try running flow --help')
                        exit()
